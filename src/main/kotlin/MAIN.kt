//import com.amazonaws.auth.AWSStaticCredentialsProvider
//import com.amazonaws.auth.BasicSessionCredentials
//import com.amazonaws.services.s3.AmazonS3
//import com.amazonaws.services.s3.AmazonS3ClientBuilder
//
//fun main() {
//    val accessKeyId = "ASIASEF4EPOOQOPYR77F"
//    val secretAccessKey = "ndyFt7znHn2paHsWYsalPfV9XqWCdTx6QhMZy4gF"
//    val sessionToken = "" // Replace <TOKEN_VALUE> with your actual token
//
//    val sessionCredentials = BasicSessionCredentials(
//        accessKeyId,
//        secretAccessKey,
//        sessionToken
//    )
//
//    val s3Client: AmazonS3 = AmazonS3ClientBuilder.standard()
//        .withCredentials(AWSStaticCredentialsProvider(sessionCredentials))
//        .build()
//
//    // Use s3Client to perform your AWS operations, for example, listing buckets
//    val buckets = s3Client.listBuckets()
//    println("Your Amazon S3 buckets are:")
//    buckets.forEach { b -> println("* " + b.name) }
//}